...
function CEKIP () {
    ...
    IPVPS=$(curl -sS https://raw.githubusercontent.com/hunter-tun/permission/main/ip | grep $MYIP | awk '{print $4}')
    if [[ $MYIP == $IPVPS ]]; then
        domain
        Casper2
    else
...
echo -e "${tyblue}│  [ 2 ]  \033[1;37mDomain xxx.premm.my.id          ${NC}"                                        
echo -e "${tyblue}└──────────────────────────────────────────┘${NC}"
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/hunter-tun/permission/main/ip | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
